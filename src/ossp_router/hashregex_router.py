...
In-package copy of baselines/hash_regex.py so the container image (which
copies only src/) can run the trained router; adds a bundled default
artifact path so the fixed container CLI needs no --artifact argument.
"""
...
PREMIUM_AX31_FILL_SAFETY_RATIO = 0.40
...
def _default_artifact_path() -> Path:
    return Path(__file__).resolve().parent / "resources" / "router-artifact.v1.json"


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="router-run",
        description="학습된 regex·feature-hashing 라우터",
    )
